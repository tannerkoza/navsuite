...
class GeoplotData:
    ...
    def __post_init__(self):
        ...
        if not self.deg:
            self.lat = np.degrees(self.lat)
            self.lon = np.degrees(self.lon)


def igeoplot(
    geoplot_data: list[GeoplotData],
    symbol: str = "circle",
    size: int = 5,
...
def test():
    trajectory = load_sample_trajectory(trajectory_name="flight_usa_sdx_01s_onego")
    ...
    translated_trajectory = translate_trajectory(
        lat=trajectory[1],
        lon=trajectory[2],
        alt=trajectory[3],
        lat0=np.degrees(lla0.lat),
        lon0=np.degrees(lla0.lon),
        alt0=lla0.alt,
        deg=True,
    )
    ...
    geoplot_data = [
        ...
        GeoplotData(
            label="Shifted Sample Flight",
            lat=np.degrees(translated_trajectory.lat),
            lon=np.degrees(translated_trajectory.lon),
            alt=translated_trajectory.alt,
            time=trajectory[0],
        ),
    ]

    igeoplot(geoplot_data=geoplot_data)
